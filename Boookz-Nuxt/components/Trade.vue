...
// console.log(loggedInUserOfferedBook.value)
// console.log(loggedInUserOfferedBook)
...
if (clickedBookShelf === "wanted") {
    ...
    tradeMessage.value = "Choose one of the book given-away by " + profileStore.userName + " :))";
} else {
...
function toggleSelect(book) {
  ...
  // toggleSelect: if book.selected is false, loop through the store.userGiveAwayBooks array
    ...
    if (clickedBookShelf === "giveaway") {
      userStore.userGiveAwayBooks.forEach((b) => {b.selected = false;});
    } else {
      profileStore.userGiveAwayBooks.forEach((b) => {b.selected = false;});
    }
    book.selected = true;
...
function confirmChoice() {
  if (clickedBookShelf === "giveaway") {
    ...
    userStore.startTransaction(userStore.chosenBookForATrade.pk, selectedBook.value.pk);
  }
}
</script>
